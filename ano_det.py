...
threshold = np.mean(mse) + 6* np.std(mse)

# Identify anomalies
anomalies = test_data[mse > threshold]

# Plot the data
plt.scatter(test_data[:, 20], test_data[:, 24], color='blue', label='Normal')
plt.scatter(anomalies[:, 20], anomalies[:, 24], color='red', label='Anomaly')
